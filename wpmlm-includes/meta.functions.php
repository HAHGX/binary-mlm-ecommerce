...
function wpmlm_sanitize_meta_key( $key )
{
 ...
 * @internal
 */
function wpmlm_get_meta( $object_id = 0, $meta_key, $type) {
	global $wpdb;
	$cache_object_id = $object_id = (int)$object_id;
	$object_type = $type;
	$value = wp_cache_get( $cache_object_id, $object_type );
	$meta_key = wpmlm_sanitize_meta_key( $meta_key );
	$meta_tuple = compact( 'object_type', 'object_id', 'meta_key', 'meta_value', 'type' );
	$meta_tuple = apply_filters( 'wpmlm_get_meta', $meta_tuple );
	extract( $meta_tuple, EXTR_OVERWRITE );
	$meta_value = $wpdb->get_var( $wpdb->prepare( "SELECT `meta_value` FROM `".WPMLM_TABLE_META."` WHERE `object_type` = %s AND `object_id` = %d AND `meta_key` = %s", $object_type, $object_id, $meta_key ) );
 ...
 * @internal
 */
function wpmlm_update_meta( $object_id = 0, $meta_key, $meta_value, $type, $global = false ) {
	global $wpdb;
	...
	$meta_tuple = compact( 'object_type', 'object_id', 'meta_key', 'meta_value', 'type' );
	$meta_tuple = apply_filters( 'wpmlm_update_meta', $meta_tuple );
	extract( $meta_tuple, EXTR_OVERWRITE );

	$meta_value = $_meta_value = maybe_serialize( $meta_value );
	$meta_value = maybe_unserialize( $meta_value );

	$cur = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM `".WPMLM_TABLE_META."` WHERE `object_type` = %s AND `object_id` = %d AND `meta_key` = %s", $object_type, $object_id, $meta_key ) );
	if ( !$cur ) {
		$wpdb->insert( WPMLM_TABLE_META, array( 'object_type' => $object_type, 'object_id' => $object_id, 'meta_key' => $meta_key, 'meta_value' => $_meta_value ) );
	} elseif ( $cur->meta_value != $meta_value ) {
		$wpdb->update( WPMLM_TABLE_META, array( 'meta_value' => $_meta_value), array( 'object_type' => $object_type, 'object_id' => $object_id, 'meta_key' => $meta_key ) );
	}
 ...
 * @internal
 */
function wpmlm_delete_meta( $object_id = 0, $meta_key, $meta_value, $type, $global = false ) {
	global $wpdb;
	if ( !is_numeric( $object_id ) || empty( $object_id ) && !$global )
		return false;
	...
	$meta_tuple = compact( 'object_type', 'object_id', 'meta_key', 'meta_value', 'type' );
	$meta_tuple = apply_filters( 'wpmlm_delete_meta', $meta_tuple );
	extract( $meta_tuple, EXTR_OVERWRITE );

	$meta_value = maybe_serialize( $meta_value );

	if ( empty( $meta_value ) )
		$meta_sql = $wpdb->prepare( "SELECT `meta_id` FROM `".WPMLM_TABLE_META."` WHERE `object_type` = %s AND `object_id` = %d AND `meta_key` = %s", $object_type, $object_id, $meta_key );
	else
		$meta_sql = $wpdb->prepare( "SELECT `meta_id` FROM `".WPMLM_TABLE_META."` WHERE `object_type` = %s AND `object_id` = %d AND `meta_key` = %s AND `meta_value` = %s", $object_type, $object_id, $meta_key, $meta_value );

	if ( !$meta_id = $wpdb->get_var( $meta_sql ) )
		return false;

	$wpdb->query( $wpdb->prepare( "DELETE FROM `".WPMLM_TABLE_META."` WHERE `meta_id` = %d", $meta_id ) );
	wp_cache_delete( $cache_object_id, $object_type );
	return true;
}
...
function wpmlm_get_cartmeta( $cart_id, $meta_key ) {
	return wpmlm_get_meta( $cart_id, $meta_key, 'wpmlm_cart_item' );
}

function wpmlm_update_cartmeta( $cart_id, $meta_key, $meta_value ) {
	return wpmlm_update_meta( $cart_id, $meta_key, $meta_value, 'wpmlm_cart_item' );
}

function wpmlm_delete_cartmeta( $cart_id, $meta_key, $meta_value = '' ) {
	return wpmlm_delete_meta( $cart_id, $meta_key, $meta_value, 'wpmlm_cart_item' );
}
/**
...
function wpmlm_get_categorymeta( $cart_id, $meta_key ) {
	return wpmlm_get_meta( $cart_id, $meta_key, 'wpmlm_category' );
}
...
function wpmlm_delete_categorymeta( $cart_id, $meta_key, $meta_value = '' ) {
	return wpmlm_delete_meta( $cart_id, $meta_key, $meta_value, 'wpmlm_category' );
}
/**
...
class wpmlm_custom_meta {
	...
	function wpmlm_custom_meta($postid) {
		global $wpdb;
