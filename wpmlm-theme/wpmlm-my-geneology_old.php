...
class BinaryTree 
{
	...
	function BinaryTree()
	{
		$this->add_page_id = $this->get_page_id('registration-page');
		$this->view_page_id = $this->get_page_id('my-geneology');
	}
	...
	function network()
	{
		...
		$res12 = mysql_fetch_array(mysql_query("SELECT * FROM ".WPMLM_TABLE_USER." WHERE username = '".$username2."'"));
		if($_POST['search'])
		{
			$username = $_POST['username'];
			$select = mysql_query("SELECT user_key FROM ".WPMLM_TABLE_USER." WHERE username = '".$username."'");
			$num = mysql_num_rows($select);
			if($num)
			{
				$result = mysql_fetch_array($select);
				$left = mysql_query("SELECT pkey FROM ".WPMLM_TABLE_RIGHT_LEG." 
				...
				{
					$right = mysql_query("SELECT pkey FROM ".WPMLM_TABLE_LEFT_LEG." 
										  WHERE ukey = '".$result['user_key']."' AND pkey = '".$res12['user_key']."'");
...
/*
		...
		if(!checkKey($key))
		{?>
		<p style="margin:0 auto;padding:0px;font-family:Arial, Helvetica, sans-serif;font-size:20px; text-align:center; font-weight:bold; padding:25px 0px 15px 0px;color:grey;">There has been an error while generating this tree. <br>Please contact the system admin at to report this problem.</p>
		<?php
			exit;
		}
...
$res = mysql_fetch_array(mysql_query("SELECT user_key FROM ".WPMLM_TABLE_USER." WHERE username = '".$username1."'"));
?>
<style type="text/css">
	span.owner
	{
