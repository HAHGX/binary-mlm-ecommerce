...
jQuery(document).ready(function(){
	...
	jQuery(function() {
		...
		if (typeof jQuery('#wpmlm_options > ul#tabs').tabs != "undefined") {
			$tabs = jQuery('#wpmlm_options > ul#tabs').tabs();
		}
		// 			current_tab = window.location.href.split('#');

		// this here code handles remembering what tab you were on
		jQuery('#wpmlm_options > ul').bind('tabsselect', function(event, ui) {
		...
		jQuery('#wpmlm_options > ul').bind('tabsload', function(event, ui) {
			bind_shipping_rate_deletion();
			// 				form_action = jQuery('#cart_options').attr('action').split('#');  //split at the #
			// 				form_action = form_action[0]+"#"+ui.panel.id; // get the first item, add the hash then our current tab ID
			// 				jQuery('#cart_options').attr('action', form_action); // stick it all back in the action attribute
...
var getresults=function(results) {
	...
	jQuery("#gallery_list").sortable({
		...
		update: function (e,ui){
			...
			jQuery('#gallery_image_'+set[0]).append("<a class='editButton'>Edit   <img src='" + WPMLM_CORE_IMAGES_URL + "/pencil.png' alt='' /></a>");
			jQuery('#gallery_image_'+set[0]).parent('li').attr('id', 0);
			//for(i=1;i<set.length;i++) {
			//	jQuery('#gallery_image_'+set[i]).children('a.editButton').remove();
			//	jQuery('#gallery_image_'+set[i]).append("<img alt='-' class='deleteButton' src='" + WPMLM_CORE_IMAGES_URL + "cross.png'/>");
			//}
	...
	jQuery(".add_level").click(
		function() {
			jQuery(this).parent().children('table').append('<tr><td><input type="text" size="10" value="" name="productmeta_values[table_rate_price][quantity][]"/> and above</td><td><input type="text" size="10" value="" name="productmeta_values[table_rate_price][table_price][]"/></td><td><img src="' + WPMLM_CORE_IMAGES_URL + '/cross.png" class="remove_line"></td></tr>');
		}
		);
	...
	jQuery("#add_label").click(
		function(){
			jQuery("#labels").append("<br><table><tr><td>"+TXT_WPMLM_LABEL+" :</td><td><input type='text' name='productmeta_values[labels][]'></td></tr><tr><td>"+TXT_WPMLM_LIFE_NUMBER+" :</td><td><input type='text' name='productmeta_values[life_number][]'></td></tr><tr><td>"+TXT_WPMLM_ITEM_NUMBER+" :</td><td><input type='text' name='productmeta_values[item_number][]'></td></tr><tr><td>"+TXT_WPMLM_PRODUCT_CODE+" :</td><td><input type='text' name='productmeta_values[product_code][]'></td></tr><tr><td>"+TXT_WPMLM_PDF+" :</td><td><input type='file' name='productmeta_values[product_pdf][]'></td></tr></table>");
		}
		);
...
function fillvariationform(variation_id) {
	...
	jQuery('.loadingimage').attr('src',  WPMLM_CORE_IMAGES_URL + 'indicator.gif');
	jQuery('#loadingindicator_span').css('visibility','visible');
}
...
function add_variation_value(value_type) {
	...
	new_element_contents += " <a class='image_link' href='#' onclick='remove_variation_value_field(\""+new_element_id+"\")'><img src='" + WPMLM_CORE_IMAGES_URL + "trash.gif' alt='"+TXT_WPMLM_DELETE+"' title='"+TXT_WPMLM_DELETE+"' /></a><br />";
...
*/
	...
	jQuery('a#close_news_box').click( function () {
		jQuery('div.wpmlm_news').css( 'display', 'none' );
...
function wpmlm_save_postboxes_state(page, container) {
...
jQuery(document).ready(function(){
	...
	if (typeof SWFUpload != "undefined") {
		var swfu = new SWFUpload({
			flash_url : WPMLM_CORE_JS_URL + '/swfupload.swf',
			upload_url: base_url+'/?action=wpmlm_add_image',
...
function addlayer(){
	jQuery("tr.addlayer").before("<tr class='rate_row'><td><i style='color:grey'>"+TXT_WPMLM_IF_PRICE_IS+"</i><input type='text' name='layer[]' size='10'> <i style='color:grey'>"+TXT_WPMLM_AND_ABOVE+"</i></td><td><input type='text' name='shipping[]' size='10'>&nbsp;&nbsp;<a href='#' class='delete_button nosubmit' >"+TXT_WPMLM_DELETE+"</a></td></tr>");
	bind_shipping_rate_deletion();
}

function addweightlayer(){
	jQuery("tr.addlayer").before("<tr class='rate_row'><td><i style='color:grey'>"+TXT_WPMLM_IF_WEIGHT_IS+"</i><input type='text' name='weight_layer[]' size='10'> <i style='color:grey'>"+TXT_WPMLM_AND_ABOVE+"</i></td><td><input type='text' name='weight_shipping[]' size='10'>&nbsp;&nbsp;<a href='#' class='delete_button nosubmit' >"+TXT_WPMLM_DELETE+"</a></td></tr>");
	bind_shipping_rate_deletion();
}
...
function imageUploadSuccess (file, results) {
	...
	if (id == null ) {
		if(replacement_src != null) {
			...
			var img = jQuery('<div class="previewimage" id="'+id+'"><a href="'+WPMLM_IMAGE_URL+src+'" rel="product_extra_image_'+id+'" class="thickbox"><img src="'+WPMLM_IMAGE_URL+src+'" width="60" height="60" class="previewimage" /></a></div>').appendTo(this.targetHolder).hide();
			set = jQuery("#gallery_list", context).sortable('toArray');

			jQuery('#gallery_image_0', context).append("<a class='editButton'>Edit   <img src='" + WPMLM_IMAGE_URL + "pencil.png'/></a>");
		...
		var img = jQuery('<div class="previewimage" id="'+div_id+'"><input type="hidden" name="images[]" value="'+src+'"><a href="'+WPMLM_IMAGE_URL+src+'" rel="product_extra_image_'+id+'" class="thickbox"><img src="'+WPMLM_IMAGE_URL+src+'" width="60" height="60" class="previewimage" /></a></div>').appendTo(this.targetHolder).hide();

		jQuery('#gallery_image_0', context).append("<a class='editButton'>Edit   <img src='"+WPMLM_CORE_IMAGES_URL+"/pencil.png'/></a>");
		jQuery('#gallery_image_0', context).parent('li').addClass('first');
		jQuery('#gallery_image_0', context).parent('li').attr('id', 0);
		jQuery('#gallery_image_0 img.deleteButton', context).remove();
	...
	jQuery(this.progressBar).animate({
		...
		jQuery(this).parent().fadeOut(500,function() {
			...
			jQuery(img).append('<img class="deleteButton" src="'+WPMLM_CORE_IMAGES_URL+'/cross.png" alt="-" style="display: none;"/>');
			enablebuttons()
	...
	jQuery("a.delete_primary_image").click(
		function(){
			...
			if (r==true) {
				...
				set = jQuery("#gallery_list").sortable('toArray');
				jQuery('#gallery_image_'+set[0]).children('img.deleteButton').remove();
				jQuery('#gallery_image_'+set[0]).append("<a class='editButton'>Edit   <img src='"+WPMLM_CORE_IMAGES_URL+"/pencil.png'/></a>");
...
jQuery(document).ready(function(){
	jQuery(".wpmlm-row-actions").parent().parent("tr").mouseover(
		function() {
			jQuery(this).children("td").children(".wpmlm-row-actions").css("visibility", "visible");
		}
		).mouseout(
		function() {
			jQuery(this).children("td").children(".wpmlm-row-actions").css("visibility", "hidden");
		}
		);
	/*
	jQuery(".wpmlm-shipping-actions").hide();
	jQuery("#wpmlm_shipping_options").hover(
		function() {
			alert('hovering');
			jQuery(this).children(".wpmlm-shipping-actions").css("visibility", "visible");
		}
	);
*//*
.mouseout(
		function() {
			jQuery(this).children(".wpmlm-shipping-actions").css("visibility", "hidden");
		}
	);
*/
	...
	jQuery("#add_label").click(
		function(){
			jQuery("#labels").append("<br><table><tr><td>"+TXT_WPMLM_LABEL+" :</td><td><input type='text' name='productmeta_values[labels][]'></td></tr><tr><td>"+TXT_WPMLM_LABEL_DESC+" :</td><td><textarea name='productmeta_values[labels_desc][]'></textarea></td></tr><tr><td>"+TXT_WPMLM_LIFE_NUMBER+" :</td><td><input type='text' name='productmeta_values[life_number][]'></td></tr><tr><td>"+TXT_WPMLM_ITEM_NUMBER+" :</td><td><input type='text' name='productmeta_values[item_number][]'></td></tr><tr><td>"+TXT_WPMLM_PRODUCT_CODE+" :</td><td><input type='text' name='productmeta_values[product_code][]'></td></tr><tr><td>"+TXT_WPMLM_PDF+" :</td><td><input type='file' name='productmeta_values[product_pdf][]'></td></tr></table>");
		}
		);
	jQuery(".add_level").click(
		function() {
...
function wpmlm_upload_switcher(target_state) {
	switch(target_state) {
		case 'flash':
