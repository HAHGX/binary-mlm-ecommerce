...
function wpmlm_fileDialogStart() {
	jQuery("#media-upload-error").empty();
}

// progress and success handlers for media multi uploads
function wpmlm_fileQueued(fileObj) {
...
function wpmlm_uploadStart(fileObj) {return true; }

function wpmlm_uploadProgress(fileObj, bytesDone, bytesTotal) {
...
function wpmlm_prepareMediaItem(fileObj, serverData) {
...
function wpmlm_prepareMediaItemInit(fileObj) {
...
function wpmlm_itemAjaxError(id, html) {
...
function wpmlm_deleteSuccess(data, textStatus) {
	if ( data == '-1' )
...
function wpmlm_deleteError(X, textStatus, errorThrown) {
	// TODO
}

function wpmlm_updateMediaForm() {
	storeState();
...
function wpmlm_uploadSuccess(fileObj, serverData) {
  ...
  eval(serverData);
	if(upload_status == 1 ) {
		...
		output_html += "		<a class='thickbox' rel='product_extra_image_"+image_id+"' href='admin.php?wpmlm_admin_action=crop_image&amp;imagename="+image_src+"&amp;imgheight=480&amp;imgwidth=600&amp;product_id=103&amp;width=640&amp;height=342' id='extra_preview_link_"+image_id+"'>\n";
		output_html += "		<img title='Preview' alt='Preview' src='"+image_src+"' class='previewimage'/>\n";
		output_html += "		</a>\n";
		output_html += "	<img src='" + WPMLM_CORE_IMAGES_URL + "/cross.png' class='deleteButton' alt='-' style='display: none;'/>\n";
		output_html += "	</div>\n";
		output_html += "</li>\n";
...
function wpmlm_uploadComplete(fileObj) {
...
// generic message
function wpmlm_wpQueueError(message) {
	jQuery('#media-upload-error').show().text(message);
}

// file-specific message
function wpmlm_wpFileError(fileObj, message) {
...
function wpmlm_fileQueueError(fileObj, error_code, message)  {
...
function wpmlm_fileDialogComplete(num_files_queued) {
	try {
...
function wpmlm_swfuploadPreLoad() {
