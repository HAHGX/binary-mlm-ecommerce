(function($){
	if( pagenow == 'edit-wpmlm_product_category' ) {
		...
		$(function(){
			...
			table.find('tbody tr').each(function(){
				...
				t.data('level', WPMLM_Term_List_Levels[id]);
				t.data('id', id);
			});
			table.wpmlm_sortable_table({
				stop : category_sort
			});
...
jQuery(document).ready(function(){
	...
	jQuery('.wpmlm_edit_coupon').click(function(){
	...
	jQuery('.wpmlm-separator').livequery(function(){
		jQuery(this).click(function(){
			if(jQuery('#wpmlm-col-left').css('width') == '20px'){
				left_col_width = '50%';
				right_col_width = '48%';
			...
			jQuery('#wpmlm-col-right').animate(
			{
	...
	var firstclick = true
	jQuery('a.wpmlm_add_new_currency').livequery(function(){
	...
	jQuery('a.wpmlm_delete_currency_layer').livequery(function(){
		jQuery(this).click(function(event){
	...
	jQuery('form input.prdfil').livequery(function(){
		jQuery(this).click(function(event){
			...
			jQuery.post('admin.php?wpmlm_admin_action=product_files_upload',post_values, function(returned_data){
				tb_remove();
				if (returned_data.length > 0) {
					jQuery('#wpmlm_product_download_forms .select_product_file tbody').append(returned_data).
	...
	//delete currency layer in admin product page
	jQuery('a.wpmlm_mass_resize').livequery(function(){
		jQuery(this).click(function(event){
	...
	jQuery('#wpmlm_product_list .wpmlm_ie_save').live('click', function(){
		jQuery(this).parents('tr:first').find('.loading_indicator').css('visibility', 'visible');
		var id =jQuery(this).parents('tr:first').find('.wpmlm_ie_id').val();
		var title = jQuery(this).parents('tr:first').find('.wpmlm_ie_title').val();
		var weight = jQuery(this).parents('tr:first').find('.wpmlm_ie_weight').val();
		var stock = jQuery(this).parents('tr:first').find('.wpmlm_ie_stock').val();
		var price = jQuery(this).parents('tr:first').find('.wpmlm_ie_price').val();
		var special_price = jQuery(this).parents('tr:first').find('.wpmlm_ie_special_price').val();
		var sku = jQuery(this).parents('tr:first').find('.wpmlm_ie_sku').val();
		//post stuff
		var data = {
			action: 'wpmlm_ie_save',
			id: id,
			title: title,
			weight: weight,
			stock: stock,
		...
		jQuery.post(ajaxurl, data, function(response) {
			...
			if(response.error){
				...
				jQuery('#post-' + response.id + ' td input.wpmlm_ie_field, #post-' + response.id + ' td .wpmlm_inline_actions').show();
    ...
    //As far as I can tell, WP provides no good way of unsetting elements in the bulk edit area...tricky jQuery action will do for now....not ideal whatsoever, nor eternally stable.
     if( pagenow == 'edit-wpmlm-product' ) {
        ...
        jQuery('fieldset.inline-edit-col-left.wpmlm-cols').css({'float': 'right', 'clear' : 'right'});
        ...
        jQuery("ul.wpmlm-variation-checklist").css('display', 'none');
        jQuery("div.inline-edit-date").css('display', 'none');
    }
        if( wpmlm_adminL10n.dragndrop_set == "true" && typenow == "wpmlm-product" && adminpage == "edit-php" ) {
            // this makes the product list table sortable
            jQuery('table.widefat:not(.tags)').sortable({
		update: function(event, ui) {
			category_id = jQuery('select#wpmlm_product_category option:selected').val();
			product_order = jQuery('table.widefat').sortable( 'serialize' );
			post_values = "category_id="+category_id+"&"+product_order;
			jQuery.post( 'index.php?wpmlm_admin_action=save_product_order', post_values, function(returned_data) { });
		},
		items: 'tbody tr',
		axis: 'y',
		...
		placeholder: 'product-placeholder',
                cursor: 'move',
                cancel: 'tr.inline-edit-wpmlm-product'
            });
	}
	...
	jQuery("tr.product-edit").hover(
		function() {
			jQuery(".wpmlm-row-actions", this).css("visibility", "visible");
		},
		function() {
			jQuery(".wpmlm-row-actions", this).css("visibility", "hidden");
		}
		);
	...
	jQuery("#gallery_list").livequery(function(){
		jQuery(this).sortable({
			...
			update: function (e,ui){
				...
				jQuery.post( 'index.php?wpmlm_admin_action=rearrange_images', postVars, function(returned_data) {
					eval(returned_data);
	...
	jQuery('.wpmlm_featured_product_toggle').livequery(function(){
		jQuery(this).click(function(event){
	...
	jQuery('.inline-editor').livequery(function() {
		...
		jQuery(this).find('.wpmlm_ie_weight').val(weight);
		jQuery(this).find('.wpmlm_ie_stock').val(stock);
		jQuery(this).find('.wpmlm_ie_price').val(price);
		jQuery(this).find('.wpmlm_ie_sale_price').val(sale_price);
		jQuery(this).find('.wpmlm_ie_sku').val(sku);
	});
});
...
*/
function wpmlm_edToolbar() {
	//document.write('<div id="ed_toolbar">');
	output = '';
	for (i = 0; i < edButtons.length; i++) {
		output += 	wpmlm_edShowButton(edButtons[i], i);
	}
...
function wpmlm_edShowButton(button, i) {
	if (button.id == 'ed_img') {
...
//ajax.post("index.php",gercurrency,"wpmlm_admin_action=change_currency&currencyid="+id);
}
...
function show_status_box(id,image_id) {
	...
	if(state != 'block') {
		document.getElementById(id).style.display = 'block';
		document.getElementById(image_id).src =  WPMLM_CORE_IMAGES_URL + '/icon_window_collapse.gif';
	} else {
		document.getElementById(id).style.display = 'none';
		document.getElementById(image_id).src =  WPMLM_CORE_IMAGES_URL + '/icon_window_expand.gif';
	}
	return false;
}
...
function editinline_get_id(){
	...
	if( has_var == '1'){
   		jQuery(".wpmlm-cols").css('display', 'none');
		jQuery(".wpmlm-cols:first").text('Quick Edit options are limited when editing products that have variations. You will need to edit the variations themselves.')
		.addClass('wpmlm_var_description')
		.css('display','block');
	}else{
  		jQuery(".wpmlm-cols").css('display', 'block');
	}
