...
class WPMLM_Settings_Tab_Taxes extends WPMLM_Settings_Tab
{
	public function __construct() {
	}

	public function callback_submit_options() {
		$taxes_enabled = ( isset( $_POST['wpmlm_options']['wpec_taxes_enabled'] ) ) ? 1 : 0;
		...
		foreach ( $taxes_rates_types as $taxes_type ) {
			...
			//check the rates
			if ( isset( $_POST['wpmlm_options']['wpec_taxes_' . $taxes_type] ) ) {
				foreach ( $_POST['wpmlm_options']['wpec_taxes_' . $taxes_type] as $tax_rate ) {
	...
	?>
				<h3><?php _e( 'Tax Settings', 'wpmlm' ); ?></h3>
				<p>
					<label for='wpec_taxes_enabled'>
						<input <?php if ( $wpec_taxes_options['wpec_taxes_enabled'] ) {
			echo 'checked="checked"';
		} ?> type="checkbox" id='wpec_taxes_enabled' name='wpmlm_options[wpec_taxes_enabled]' />
	<?php _e( 'Turn tax on', 'wpmlm' ); ?>
				</label>
			</p>
			<p>
			...
			echo 'checked="checked"';
		} ?> type="radio" value='exclusive' id='wpec_taxes_inprice1' name='wpmlm_options[wpec_taxes_inprice]' />
	<?php _e( 'Product prices are tax exclusive - add tax to the price during checkout', 'wpmlm' ); ?>
				</label>
			</p>
			<p>
			...
			echo 'checked="checked"';
		} ?> type="radio" value='inclusive' id='wpec_taxes_inprice2' name='wpmlm_options[wpec_taxes_inprice]' />
	<?php _e( "Product prices are tax inclusive - during checkout the total price doesn't increase but tax is shown as a line item", 'wpmlm' ); ?>
				</label>
			</p>
			<h4><?php _e( 'Product Specific Tax', 'wpmlm' ); ?></h4>
			<p>
				<label for='wpec_taxes_product_1'>
					<input <?php if ( $wpec_taxes_options['wpec_taxes_product'] == 'add' ) {
			echo 'checked="checked"';
		} ?> type="radio" value='add' id='wpec_taxes_product_1' name='wpmlm_options[wpec_taxes_product]' />
	<?php _e( 'Add per product tax to tax percentage if product has a specific tax rate', 'wpmlm' ); ?>
				</label>
			</p>
			<p>
			...
			echo 'checked="checked"';
		} ?> type="radio" value='replace' id='wpec_taxes_product_2' name='wpmlm_options[wpec_taxes_product]' />
	<?php _e( 'Replace tax percentage with product specific tax rate', 'wpmlm' ); ?>
				</label>
			</p>

			<h4><?php _e( 'Tax Logic', 'wpmlm' ); ?></h4>
			<p>
				<label for='wpec_taxes_logic_1'>
					<input <?php if ( $wpec_taxes_options['wpec_taxes_logic'] == 'billing_shipping' ) {
							echo 'checked="checked"';
						} ?> type="radio" value='billing_shipping' id='wpec_taxes_logic_1' name='wpmlm_options[wpec_taxes_logic]' />
						<?php _e( 'Apply tax when Billing and Shipping Country is the same as Tax Rate', 'wpmlm' ); ?>
				</label>
	            ...
	            <p>
					<label for='wpec_billing_preference'>
						<input <?php if ( $wpec_taxes_options['wpec_taxes_logic'] == 'billing_shipping' && $wpec_taxes_options['wpec_billing_shipping_preference'] == 'billing_address' ) {
							echo 'checked="checked"';
						} ?> type="radio" value='billing_address' id='wpec_billing_preference' name='wpmlm_options[wpec_billing_shipping_preference]' />
					<?php _e( 'Apply tax to Billing Address', 'wpmlm' ); ?>
						</label>
		            </p>
		            <p>
						<label for='wpec_shipping_preference'>
							...
							echo 'checked="checked"';
						} ?> type="radio" value='shipping_address' id='wpec_shipping_preference' name='wpmlm_options[wpec_billing_shipping_preference]' />
	<?php _e( 'Apply tax to Shipping Address', 'wpmlm' ); ?>
						</label>
		            </p>
				</div>
				</p>
				<p>
					<label for='wpec_taxes_logic_2'>
						<input <?php if ( $wpec_taxes_options['wpec_taxes_logic'] == 'billing' ) {
							echo 'checked="checked"';
						} ?> type="radio" value='billing' id='wpec_taxes_logic_2' name='wpmlm_options[wpec_taxes_logic]' />
						<?php _e( 'Apply tax when Billing Country is the same as Tax Rate', 'wpmlm' ); ?>
				</label>
			</p>
			<p>
				<label for='wpec_taxes_logic_3'>
					<input <?php if ( $wpec_taxes_options['wpec_taxes_logic'] == 'shipping' ) {
							echo 'checked="checked"';
						} ?> type="radio" value='shipping' id='wpec_taxes_logic_3' name='wpmlm_options[wpec_taxes_logic]' />
						<?php _e( 'Apply tax when Shipping Country is the same as Tax Rate', 'wpmlm' ); ?>
				</label>
			</p>
			<div id='metabox-holder' class="metabox-holder">
				<div id='wpec-taxes-rates-container' class='postbox'>
					<h3 class='hndle' style='cursor: default'><?php _e( 'Tax Rates', 'wpmlm' ); ?></h3>
	...
	?>
						<!--End Taxes Output-->
						<p id="wpmlm-add-tax-rates">
							<a href="#"><?php _e( 'Add New Tax Rate', 'wpmlm' ); ?></a>
							<img src="<?php echo esc_url( admin_url( 'images/wpspin_light.gif' ) ); ?>" class="ajax-feedback" title="" alt="" />
						</p>
					</div>
				</div>
				<div id='wpec-taxes-bands-container' class='postbox'>
					<h3 class='hndle' style='cursor: default'><?php _e( 'Tax Bands', 'wpmlm' ); ?></h3>
					<div id='wpec-taxes-bands' class='inside'>

	<?php
						echo '<p>' . __( 'Note: Tax Bands are special tax rules you can create and apply on a per-product basis. <br /> Please visit the product page to apply your Tax Band.', 'wpmlm' ) . '</p>';

						//echo message regarding inclusive tax
						if ( !$wpec_taxes_controller->wpec_taxes_isincluded() ) {
							echo '<p>' . __( 'Note: Tax Bands do not take affect when product prices are tax exclusive.', 'wpmlm' ) . '</p>';
						}// if
	...
	?>
						<p id="wpmlm-add-tax-bands">
							<a href="#"><?php _e( 'Add New Tax Band', 'wpmlm' ); ?></a>
							<img src="<?php echo esc_url( admin_url( 'images/wpspin_light.gif' ) ); ?>" class="ajax-feedback" title="" alt="" />
									</p>
								</div>
