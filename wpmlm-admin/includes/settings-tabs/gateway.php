...
class WPMLM_Settings_Tab_Gateway extends WPMLM_Settings_Tab
{
	...
	public function __construct() {
		if ( isset( $_REQUEST['payment_gateway_id'] ) )
			update_user_option( get_current_user_id(), 'wpmlm_settings_selected_payment_gateway', $_REQUEST['payment_gateway_id'] );
	...
	private function get_gateway_form( $selected_gateway ) {
		...
		if ( $selected_gateway_data ) {
			...
			?>
				<tr>
					<td><?php esc_html_e( 'Display Name', 'wpmlm' ); ?></td>
					<td>
						<input type="text" name="user_defined_name[<?php echo esc_attr( $selected_gateway ); ?>]" value="<?php echo esc_html( $display_name ); ?>" /><br />
						<small><?php esc_html_e( 'The text that people see when making a purchase.', 'wpmlm' ); ?></small>
					</td>
				</tr>
			<?php
			$output = ob_get_clean();
	...
	public function display_payment_gateway_settings_form() {
		$selected_gateway = (string) get_user_option( 'wpmlm_settings_selected_payment_gateway', get_current_user_id() );
		...
		if ( ! $payment_data ) {
			$payment_data = array(
				'name'        => __( 'Edit Gateway Settings', 'wpmlm' ),
				'form_fields' => __( 'Modify a payment gateway settings by clicking "Edit" link on the left.', 'wpmlm' ),
			);
		}
		?>
		<td id='wpmlm-payment-gateway-settings-panel' class='wpmlm-module-settings' rowspan='2'>
			<div class='postbox'>
	...
	?>
		...
		<div class='metabox-holder'>
			...
			<input type='hidden' name='wpmlm_gateway_settings' value='gateway_settings' />
			<?php
			if ( get_option( 'custom_gateway' ) == 1 ) {
			 ...
			 ?>
			<table id='wpmlm-payment-gateway-settings' class='wpmlm-edit-module-options'>
				<tr>
					<td>
						<div class='postbox'>
							<h3 class='hndle'><?php _e( 'Select Payment Gateways', 'wpmlm' ); ?></h3>
							<div class='inside'>
								<p><?php _e( 'Activate the payment gateways that you want to make available to your customers by selecting them below.', 'wpmlm' ); ?></p>
								<br />
								<?php
								...
								?>
									...
									<div class="wpmlm-select-gateway">
										<div class='wpmlm-gateway-actions'>
											<span class="edit">
													<a class='edit-payment-module' data-gateway-id="<?php echo esc_attr( $gateway['internalname'] ); ?>" title="<?php _e( "Edit this Payment Gateway's Settings", 'wpmlm' ) ?>" href='<?php echo esc_attr( $this->get_gateway_settings_url( $gateway['internalname'] ) ); ?>'><?php esc_html_e( 'Edit', 'wpmlm' ); ?></a>
													<img src="<?php echo esc_url( admin_url( 'images/wpspin_light.gif' ) ); ?>" class="ajax-feedback" title="" alt="" />
											</span>
									</div>
									<p>
										<input name='wpmlm_options[custom_gateway_options][]' <?php checked( in_array( $gateway['internalname'], (array) $selected_gateways ) ); ?> type='checkbox' value='<?php esc_attr_e( $gateway['internalname'] ); ?>' id='<?php esc_attr_e( $gateway['internalname'] ); ?>_id' />
					...
					<?php }
								...
								<div class='submit gateway_settings'>
									<input type='submit' value='<?php _e( 'Update &raquo;', 'wpmlm' ) ?>' name='updateoption' />
								</div>
								</div>
							</div>

								<h4><?php _e( 'We Recommend', 'wpmlm' ); ?></h4>
								<a style="border-bottom:none;" href="https://www.paypal.com/nz/mrb/pal=LENKCHY6CU2VY" target="_blank"><img src="<?php echo WPMLM_CORE_IMAGES_URL; ?>/paypal-referal.gif" border="0" alt="Sign up for PayPal and start accepting credit card payments instantly." /></a> <br /><br />
