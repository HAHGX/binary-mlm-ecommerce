...
class WPMLM_Settings_Tab_Checkout extends WPMLM_Settings_Tab
{
	...
	public function __construct() {
		...
		$this->force_ssl = get_option( 'wpmlm_force_ssl', 0 );
		$this->checkout_sets = get_option( 'wpmlm_checkout_form_sets' );
		$this->current_checkout_set = empty( $_GET['checkout_set'] ) ? 0 : $_GET['checkout_set'];
		$this->field_types = get_option( 'wpmlm_checkout_form_fields' );
		$this->user_field_types = array('text','textarea','heading','select','radio','checkbox');

		$form_sql = $wpdb->prepare( "
			SELECT *
			FROM " . WPMLM_TABLE_CHECKOUT_FORMS . "
		...
		$columns = array(
			...
			'type'        => __('Type', 'wpmlm'),
			'unique_name' => '&nbsp;',
			'display'     => __('Display', 'wpmlm'),
			'mandatory'   => __('Mandatory', 'wpmlm'),
			'actions'     => '&nbsp;',
		);
	...
	public function callback_submit_options() {
		...
		if ( ! empty( $_POST['new_form_set'] ) ) {
			...
			update_option( 'wpmlm_checkout_form_sets', $checkout_sets );
			add_settings_error( 'wpmlm-settings', 'wpmlm_form_set_added', __( 'New form set successfully created.', 'wpmlm' ), 'updated' );
		}
		...
		$sql = $wpdb->prepare( "SELECT id FROM " . WPMLM_TABLE_CHECKOUT_FORMS . " WHERE checkout_set = %s", $this->current_checkout_set );
		...
		if ( ! empty( $_POST['form_name'] ) ) {
			foreach ( $_POST['form_name'] as $field_id => $name ) {
				...
				$wpdb->update( WPMLM_TABLE_CHECKOUT_FORMS, $data, $where, $data_format, '%d' );
			}
		}

		// delete all other fields that are not present in the submitted form
		if ( ! empty( $ids ) ) {
			$sql = "DELETE FROM " . WPMLM_TABLE_CHECKOUT_FORMS . " WHERE id IN (" . implode( ', ', $ids ) . ")";
			$wpdb->query( $sql );
		}
		...
		foreach ( $_POST['new_field_name'] as $key => $name ) {
			...
			$wpdb->insert( WPMLM_TABLE_CHECKOUT_FORMS, $data, $data_format );
		}
	}
	...
	private function prototype_field( $mode = 'hidden' ) {
		...
		<tr id="<?php echo $row_id; ?>" class="<?php echo $row_class; ?>" <?php echo $data; ?> <?php echo $style; ?>>
			<td class="drag">
				<div class="cell-wrapper">
					<a title="<?php esc_attr_e( 'Click and Drag to Order Checkout Fields', 'wpmlm' ); ?>">
						<img src="<?php echo esc_url( WPMLM_CORE_IMAGES_URL . '/drag.png' ); ?>" />
					</a>
			...
			<td class="namecol">
				<div class="cell-wrapper">
					...
					<a class="edit-options" href="#"><?php esc_html_e( 'Edit Options', 'wpmlm' ); ?></a>
				</div>
			</td>
			<td class="typecol">
				<div class="cell-wrapper">
			...
			<td class="actionscol">
				<div class="cell-wrapper">
					<a tabindex="-1" title="<?php _e( 'Delete Field', 'wpmlm' ); ?>" class="action delete" href="#">Delete</a>
					<a tabindex="-1" title="<?php _e( 'Add Field', 'wpmlm' ); ?>" class="action add" href="#">Add</a>
				</div>
			</td>
		</tr>
		<tr id="field-options-prototype" class="form-field-options">
			...
			<td>
				<div class="cell-wrapper">
					<h4></h4>
					<table class="wpmlm-field-options-table">
						<thead>
							<th class="column-labels"><?php echo esc_html_x( 'Label', "checkout field's options", 'wpmlm' ); ?></th>
							<th class="column-values"><?php echo esc_html_x( 'Value', "checkout field's options", 'wpmlm' ); ?></th>
	...
	public function display() {
		...
		$form_types = get_option('wpmlm_checkout_form_fields');
		$unique_names = get_option('wpmlm_checkout_unique_names');

		do_action('wpmlm_checkout_form_fields_page');
		?>

		<div class='metabox-holder' style='width:95%;'>
			<div class='postbox'>
				<input type='hidden' name='checkout_submits' value='true' />
				<h3 class='hndle'><?php _e( 'Misc Checkout Options' , 'wpmlm' ); ?></h3>
				<div class='inside'>
					<table>
						<tr>
							<td><?php _e('Users must register before checking out', 'wpmlm'); ?>:</td>
							<td>
							<input type='radio' value='1' name='wpmlm_options[require_register]' id='require_register1' <?php checked( $this->require_register, 1 ); ?> />
							<label for='require_register1'><?php _e('Yes', 'wpmlm');?></label> &nbsp;
							<input type='radio' value='0' name='wpmlm_options[require_register]' id='require_register2' <?php checked( $this->require_register, 0 ); ?> />
							<label for='require_register2'><?php _e('No', 'wpmlm');?></label>
							</td>
							<td>
							<a title='<?php _e('If yes then you must also turn on the wordpress option "Any one can register"', 'wpmlm');?>' class='flag_email' href='#' ><img src='<?php echo WPMLM_CORE_IMAGES_URL; ?>/help.png' alt='' /> </a>
							</td>
						</tr>

						<tr>
							<td scope="row"><?php _e('Enable Shipping Same as Billing Option', 'wpmlm'); ?>:</td>
							<td>
								<input type='radio' value='1' name='wpmlm_options[shippingsameasbilling]' id='shippingsameasbilling1' <?php checked( $this->shipping_same_as_billing, 1 ); ?> />
								<label for='shippingsameasbilling1'><?php _e('Yes', 'wpmlm');?></label> &nbsp;
								<input type='radio' value='0' name='wpmlm_options[shippingsameasbilling]' id='shippingsameasbilling2' <?php checked( $this->shipping_same_as_billing, 0 ); ?> />
								<label for='shippingsameasbilling2'><?php _e('No', 'wpmlm');?></label>
							</td>
						</tr>
						<tr>
							<td><?php _e('Force users to use SSL', 'wpmlm'); ?>:</td>
							<td>
								<input type='radio' value='1' name='wpmlm_options[wpmlm_force_ssl]' id='wpmlm_force_ssl1' <?php checked( $this->force_ssl, 1 ); ?> />
								<label for='wpmlm_force_ssl1'><?php _e('Yes', 'wpmlm');?></label> &nbsp;
								<input type='radio' value='0' name='wpmlm_options[wpmlm_force_ssl]' id='wpmlm_force_ssl2' <?php checked( $this->force_ssl, 0 ); ?> />
								<label for='wpmlm_force_ssl2'><?php _e('No', 'wpmlm');?></label>
							</td>
							<td>
								<a title='<?php _e('This can cause warnings for your users if you do not have a properly configured SSL certificate', 'wpmlm');?>' class='flag_email' href='#' ><img src='<?php echo WPMLM_CORE_IMAGES_URL; ?>/help.png' alt='' /> </a>
							</td>
						</tr>
					</table>
				</div>
		...
		<h3><?php _e('Form Fields', 'wpmlm'); ?></h3>
		<p><?php _e('Here you can customise the forms to be displayed in your checkout page. The checkout page is where you collect important user information that will show up in your purchase logs i.e. the buyers address, and name...', 'wpmlm');?></p>

		<p>
			<label for='wpmlm_form_set'><?php _e('Select a Form Set' , 'wpmlm'); ?>:</label>
			<select id='wpmlm_form_set' name='checkout_set'>
			...
			<input type='submit' value='<?php esc_attr_e( 'Filter', 'wpmlm' ); ?>' name='wpmlm_checkout_set_filter' class='button-secondary' />
			<a href='#' class='add_new_form_set'><?php esc_html_e("+ Add New Form Set", 'wpmlm'); ?></a>
		</p>

		<p class='add_new_form_set_forms'>
			<label><?php esc_html_e( "Add new Form Set", 'wpmlm' ); ?>:
			<input type="text" value="" name="new_form_set" /></label>
			<input type="submit" value="<?php _e('Add', 'wpmlm'); ?>" class="button-secondary" id="formset-add-sumbit"/>
		</p>

		<input type="hidden" name="selected_form_set" value="<?php echo esc_attr( $this->current_checkout_set ); ?>" />

		<table id="wpmlm_checkout_list" class="widefat page fixed"  cellspacing="0">
			<thead>
				<tr>
			...
			<tbody id='wpmlm_checkout_list_body'>
							...
							<td class="namecol">
								<div class="cell-wrapper">
									...
									<a
										...
										?>
									><?php esc_html_e( 'Edit Options', 'wpmlm' ); ?></a>
								</div>
							</td>
							<td class="typecol">
								<div class="cell-wrapper">
							...
							<td class="actionscol">
								<div class="cell-wrapper">
									<?php if ( ! $this->is_field_default( $form_field ) ): ?>
										<a tabindex="-1" title="<?php _e( 'Delete Field', 'wpmlm' ); ?>" class="action delete" href="#">Delete</a>
									<?php else: ?>
										<span title="<?php _e( 'Cannot Delete Default Fields', 'wpmlm' ); ?>" class="action delete">Delete</span>
									<?php endif; ?>
									<a tabindex="-1" title="<?php _e( 'Add Field', 'wpmlm' ); ?>" class="action add" href="#">Add</a>
								</div>
							</td>
